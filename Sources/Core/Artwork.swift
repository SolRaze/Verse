...
enum Artwork {
    ...
    /// TODO(later): the negative marker is permanent; "Fetch Metadata" clears it via invalidate().
    ...
    /// black (a raw average is often muddy). For the "Colour From Cover" lyrics tint.
    /// TODO(later): CIAreaAverage — one pixel, no palette clustering; swap for a vibrant-colour
