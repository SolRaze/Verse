...
enum LyricsResolver {
    ...
    static func resolve(mediaURL: URL?, title: String, artist: String,
        ...
        if let cached = try? String(contentsOf: cacheFile, encoding: .utf8) {
            ...
            // on every play (issue #4). TODO(later): permanent; delete the .lrc to retry a track.
    ...
    /// TODO(later): no SYLT support; AVFoundation doesn't expose it. LRCLIB covers synced.
